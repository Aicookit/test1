...
#容器
=Frame(root,bg='#08a62b',with=400,height=200)

#设置背景图片
image=image.open('img/img1.gif')
background_image = ImageTk.PhotoImage(image)
label_img = Label(frame,image=background_image,height=300,bg='#08a62b')
label_img.pack(anchor=CENTER)

#标签，显示信息。
labelvar = StringVar()
label = Label(frame,text='谁是卧底',textvariablr=labelvar,width=80,height=2,fg='white',bg='#08a62b')
label.pack()
labelvar.set('谁是卧底')
...
num_list = StringVar()
choice_player = ttk.Combobox(frame,widget=15,textvariable=num_list)
choice_player['values']=players_num #设置下拉列表的值
choice_player.current(0) #设置下拉列表默认显示的值，0为numberChosen['value']的下标值
...
choice_code['values']=all_players #设置下拉的值
...
def allot_undercover():
    global allot_undercover_num,players_code,undercover_code,all_players,choice_num,all_undercover
...
def is_not_twoPlayer():
    ...
    if len(players_code)==2:
        tkMessageBox.showinfo(title='谁是卧底',message='卧底胜利')
...
def analysis_votes():
    ...
    if len(other_ticket_codes)==0:
        tkMessageBox.showinfo(title='谁是卧底',message='真巧，所有人的票数居然都相同，大家重新开始描述')
        #初始化变量
        init()
        #更新玩家数据
        update(vote_players=players_code,voted_players=players_code)
    ...
    def start_game():
        ...
        try:
            ...
            if len(words_list) != 0:
                ...
                tkMessageBox.showinfo(title='谁是卧底', message='游戏即将开始，你们准备好了吗！')
                labelvar.set('请1号查看词语')

            else:
                tkMessageBox.showinfo(title='谁是卧底', message='已想不到其他任何词语了')
        except Exception:
    ...
    def check_word():
        ...
        if exception_code == 1:
            tkMessageBox.showwarning(title='谁是卧底', message='亲，请开始游戏')
            return '异常处理'
        ...
        if word_num <= len(players):
            ...
            tkMessageBox.showinfo(title='谁是卧底', message='请大家进行描述')
            # 显示当前玩家
            update(voted_players=players_code, vote_players=players_code)
    ...
    def vote():
        ...
        # 异常处理，防止未点击开始就点投票
        if exception_code == 1:
            tkMessageBox.showwarning(title='谁是卧底', message='亲，你是不是忘记了什么？请开始游戏')
            return '异常处理'
